...
type Reading = { output: string; error?: string; tick?: number };

export const initialState: Reading = { output: "" };

export const updateState = (
  event: { tick?: number; output?: string; error?: string },
  previous: Reading
): Reading => {
...
/* The face is served from the widget's own folder rather than carried in the bundle. The
   whole face is 2MB, where the subset it replaces was 4KB, and base64 in a bundle inflates
   by a third and is parsed again on every rebuild.

   The path is absolute because a page is served at /<screen>/<layer>, so a relative url
   would be looked for under the screen. It carries the folder name, which is the widget
   id, so renaming the folder means changing this line.

   DotGothic16 covers Japanese, Latin, Cyrillic and Greek, and about a third of the CJK
   ideographs, which is the Japanese set. Korean is not in it at all, and a simplified
   Chinese character the Japanese set does not share falls back to the system face. */
const FACE = "/nowplaying/dotgothic16.ttf";
